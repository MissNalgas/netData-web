...
const RegisterComponent: FC<IRegisterComponentProps> = ({
	...
	const { isLoading, handleSubmitFormUser, handleSubmitValidateEmail } =
		ValidationsRegister({ changeStep: setChangeAction });
		...
		<ContentForm className="flex overflow-y-auto h-screen pb-8 my-auto">
			<div className="m-auto">
				...
				{changeStateAction === 2 && (
					<div>
						<Steps />
						<RegisterAccountForm onSubmit={handleSubmitFormUser} />
					</div>
				)}
				{changeStateAction === 3 && (
					<ErrorImage
						...
						description={`${t(
							"register:description_error_create_account"
						)}`}
					/>
				)}
				{changeStateAction === 4 && (
